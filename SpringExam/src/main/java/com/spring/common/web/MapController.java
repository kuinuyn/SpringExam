...
public class MapController {
	...
	@RequestMapping(value="/mapMobileContentDaum")
	public String mapMobileContentDaum (HttpServletRequest request, CommandMap commandMap, HttpSession session) {
		
		return "common/map/mapMobileContentDaum";
	}
	
	@RequestMapping(value="/mapMobileContentDaum2")
	public String mapMobileContentDaum2 (HttpServletRequest request, CommandMap commandMap, HttpSession session) {
		
		return "common/map/mapMobileContentDaum2";
	}
	...
	@RequestMapping(value="/mapMobileDataKakao")
	public ModelAndView mapMobileDataKakao(HttpServletRequest request, CommandMap commandMap, HttpSession session) {
		ModelAndView mv = new ModelAndView();
		CommonUtil cu = new CommonUtil();
		...
		try {
			returnMsg = mapService.dgetListLightMobileMap(area, area2, keytype, keyword, min_x, max_x, min_y, max_y, (String)session.getAttribute("id"), dgubun, dongNm, searchGubun, lightGn);
		} catch (Exception e) {
			e.printStackTrace();
		}
