...
<script type="text/javascript">
	...
	function saveSubmit() {
		var yn = confirm("분전함 정보를 수정하시겠습니까?");
		if(yn){
			$("#slightForm").ajaxForm({
		...
		if(data != null){	
			...
			if(filesLen > 0) {
				for(i = 0; i<filesLen; i++) {
					str += "<a href=/board/fileDownload?fileNameKey="+encodeURI( files[i].file_name_key) + "&fileName=" + encodeURI(files[i].file_name) + "&filePath="+encodeURI(files[i].file_path)+">"+ files[i].file_name +"</a>";
	...
	function goToList() {
		...
		frm.action = '/equipment/distributionBoxList';
		frm.method ="post";
		frm.submit();
		//$("#slightForm").attr({action:'/equipment/distributionBoxList'}).submit();
	}
...
<div id="container">
	...
	<div id="sub">
		<div id="sub_title"><h3>분전함관리</h3></div>
		<form id="slightForm" name="slightForm">
			...
			<div id="board_view2">
				<table summary="분전함" cellpadding="0" cellspacing="0">
					<colgroup>
						<col width="20%">
						<col width="30%">
						<col width="20%">
